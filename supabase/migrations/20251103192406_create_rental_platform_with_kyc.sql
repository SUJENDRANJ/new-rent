/*
  ...
  ## Overview
  Complete database schema for a rental platform with authentication, products, rentals, categories, KYC verification, and admin features.

  ## Tables Created
  
  1. **profiles**
    - User profile information
    - Columns: id, email, full_name, avatar_url, is_admin, kyc_status, kyc_submitted_at, kyc_document_url
  ...
  3. **products**
    ...
    - Columns: id, host_id, category_id, title, description, price_per_day, image_url, location, location_url, is_available
    - Links to host profile and category
    
  4. **rentals**
    - Rental transactions between renters and hosts
    - Columns: id, product_id, renter_id, host_id, start_date, end_date, total_price, status
    - Status flow: pending → approved → active → completed (or cancelled)
  ...
  - KYC verification system for user verification
  - Location URLs for mapping integration
*/
